...
query <- 'SELECT * FROM "Open Analytics Layer"."Educação"."Qualidade da educação nos anos finais do ensino fundamental"'
...
mun <- 
  read_municipality(code_muni="all", 
                    year=2022,
                    showProgress = FALSE) |>
  filter(substr(code_muni, 1, 2) == "51") |>
  mutate(code_muni = as.character(code_muni)) |> 
  mutate(code_muni = substr(code_muni, 1, 6)) |> 
  left_join(edu_final, by= c("code_muni" = "cod_ibge"))
...
a <- ggplot() +
  geom_sf(data = mun_sf, 
          aes(fill = IDEB_medio, 
              geometry = geom)) +
  scale_fill_gradientn(colors = c('#A50026', '#F46D43', '#FEE090'),
                       values = rescale(c(3.5, 4.5, 5.5)),
                       limits = c(3.5, 5.5), 
                       breaks = c(3.5, 4.5, 5.5))+ 
  theme_minimal() +
  labs(fill = "Média do IDEB ", 
       title = "Qualidade da educação de escolas públicas nos anos finais do\nensino fundamental no Mato Grosso",
