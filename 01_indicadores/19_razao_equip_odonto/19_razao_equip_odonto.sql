WITH EQUIPAMENTOS AS (
    SELECT
        ...
        SUM(CASE WHEN IND_NSUS = '1' THEN QT_EXIST ELSE 0 END) AS quantidade_equip_nao_sus
    FROM 
        Dados.cnes.EQ eq
    WHERE 
...
SELECT 
    ...
    p.longitude,
    e.equipamento,
    e.quantidade_equip_sus,
    e.quantidade_equip_nao_sus,
    p.populacao,
    CAST(quantidade_equip_sus AS FLOAT) / CAST(p.populacao AS FLOAT) * 10000 AS equip_pop_sus,        
    CAST(quantidade_equip_nao_sus AS FLOAT) / CAST(p.populacao AS FLOAT) * 10000 AS equip_pop_nao_sus
FROM 
    EQUIPAMENTOS e
LEFT JOIN 
